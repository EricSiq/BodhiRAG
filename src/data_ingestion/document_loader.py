...
try:
    ...
    print("Warning: langchain_docling not available. Document loading will be limited.")
...
def extract_publication_data(file_path: str) -> List[Tuple[str, str]]:
    ...
    try:
        ...
        return []
def _process_single_document(item: Tuple[int, str, str], chunker, csv_file_path: str) -> List[Document]:
    i, title, url = item
...
def load_and_chunk_documents(
    ...
    """
    if not DOCLING_AVAILABLE:
        print("ERROR: langchain_docling not available. Cannot load documents.")
        print("Install with: pip install docling langchain-docling")
        return []
